(function(){
'use strict';
// Paleta inspirada en el cóndor andino (Alkamari)
const C = {
  negro: '#18181b',
  blanco: '#f4f4f5',
  oro: '#eab308',
...
// Parámetros típicos del cóndor andino
const state = {
  freq: 1.2,           // Frecuencia de aleteo (Hz)
  amp: 120,            // Amplitud angular (grados)
  wingLen: 220,        // Longitud del ala (px)
  speed: 1,
  layers: { points: true, tangents: false, curvature: false }
};
...
async function checkPython(){
  try{
    ...
    if(r.ok){pythonOnline = true; setEng(true); return;}
  }catch(e){}
...
async function computePy(){
  try{
    const r = await fetch('http://localhost:8000/api/compute', {
      method: 'POST',
...
function computeJS(){
  // Parámetros y armónicos adaptados al cóndor andino (Alkamari)
  const f = state.freq, a = state.amp, p = 1/f, w = 2*Math.PI*f;
  // Fracciones de ciclo y armónico menor (más lento y amplio)
  const fr = [0, .08, .18, .32, .5, .68, .82, .92, 1];
  const ct = fr.map(v => v*p);
  const cy = ct.map(t => a * (Math.sin(w*t) + .08*Math.sin(2*w*t)) / 1.08);
  const sp = new CubicSpline(ct, cy), sd = sp.sample(400);
  const ns = 1024, sr = 512, st = [], sy = [];
  for(let i=0; i<ns; i++){
    const t = i/sr;
    st.push(t);
    sy.push(a * (Math.sin(w*t) + .08*Math.sin(2*w*t)) / 1.08);
  }
  const ff = FourierAnalysis.fft(sy, sr);
  return {
    engine: 'js',
    params: { frequency: f, amplitude: a, period_ms: p*1000 },
    controlPoints: { t: ct, y: cy },
    spline: { t: sd.x, y: sd.y, dy: sd.dy, ddy: sd.ddy },
    coefficients: sp.coefficients.map((s,i)=>({
      segment: i, x0: s.x0, x1: s.x1, a: s.a, b: s.b, c: s.c, d: s.d
    })),
    signal: { t: st, y: sy },
    fourier: {
      frequencies: ff.frequencies,
      magnitudes: ff.magnitudes,
      fundamentalFreq: ff.fundamentalFreq,
      fundamentalPeriod_ms: ff.fundamentalPeriod*1000,
      fundamentalIndex: ff.fundamentalIndex
    }
  };
}
async function recomputeAll(){
  sim.flapFrequency = state.freq;
  sim.flapAmplitude = state.amp;
  sim.wingLength = state.wingLen;
...
function updateTable(){
  ...
  computedData.coefficients.forEach((s,i)=>{
...
function updateFourierUI(){
  ...
  document.getElementById('fourier-freq').textContent = f.fundamentalFreq.toFixed(2) + ' Hz';
  document.getElementById('fourier-period').textContent = f.fundamentalPeriod_ms.toFixed(1) + ' ms';
  // Rango biológico real del cóndor andino: 0.8–2.5 Hz
  const ok = f.fundamentalFreq >= 0.8 && f.fundamentalFreq <= 2.5;
  document.getElementById('fourier-match').textContent = ok ? '✓ Coincide' : '✗ Fuera de rango';
  document.getElementById('fourier-match').style.color = ok ? C.oro : C.rose;
  document.getElementById('rt-fourier-freq').textContent = f.fundamentalFreq.toFixed(2) + ' Hz';
  document.getElementById('rt-fourier-period').textContent = f.fundamentalPeriod_ms.toFixed(1) + ' ms';
  const rm = document.getElementById('rt-bio-match');
  rm.textContent = ok ? '✓ Coincide' : '✗ Fuera';
  rm.style.color = ok ? C.oro : C.rose;
}
...
function captureSnapshot(){
  ...
  snapshots.push({
    ...
    simT: sim.time.toFixed(3),
    angle: a.toFixed(2),
    vel: v.toFixed(0),
    acc: ac.toFixed(0),
    freq: state.freq,
    cycle: sim.cycle,
    phase: a>0?'Upstroke':'Downstroke'
  });
  let c = document.getElementById('snapshot-container');
  if(!c){
    c = document.createElement('div');
    c.id = 'snapshot-container';
    c.innerHTML = '<h3 class="sp-title">📸 Capturas (Pausa)</h3>';
    c.style.cssText = 'padding:12px;border-top:1px solid rgba(148,163,184,.08);max-height:200px;overflow-y:auto';
    document.getElementById('panel-right').appendChild(c);
  }
  const s = snapshots[snapshots.length-1], d = document.createElement('div');
  d.style.cssText = 'background:rgba(234,179,8,.05);border:1px solid rgba(234,179,8,.15);border-radius:8px;padding:8px;margin-top:6px;font-size:.68rem';
  d.innerHTML = `<div style="color:${C.oro};font-weight:700">#${s.id} — ${s.time}</div><div style="color:#94a3b8">t=${s.simT}s | θ=${s.angle}° | ω=${s.vel}°/s</div><div style="color:#94a3b8">Ciclo:${s.cycle} | ${s.phase}</div>`;
...
function drawChart(cid, cfg){
  ...
  for(const ds of cfg.datasets){
    if(ds.type==='bar'){
      ...
      for(let i=0;i<ds.x.length;i++){
        const x=toX(ds.x[i]),y0=toY(0),y=toY(ds.y[i]);
        const g=ctx.createLinearGradient(x,y,x,y0);
        g.addColorStop(0,ds.color||C.cyan);g.addColorStop(1,'rgba(34,211,238,.05)');
        ctx.fillStyle=g;ctx.fillRect(x-bw/2,Math.min(y,y0),bw,Math.abs(y-y0));
      }
      if(ds.hlIdx>=0&&ds.hlIdx<ds.x.length){
        const hx=toX(ds.x[ds.hlIdx]),hy=toY(ds.y[ds.hlIdx]);
        ctx.fillStyle=C.oro;ctx.shadowColor=C.oro;ctx.shadowBlur=10;ctx.beginPath();ctx.arc(hx,hy,6,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.font='700 12px "JetBrains Mono"';ctx.textAlign='center';ctx.fillText(ds.x[ds.hlIdx].toFixed(2)+' Hz',hx,hy-16);
      }
    }else{
    ...
    if(ds.points){for(let i=0;i<ds.points.x.length;i++){const px=toX(ds.points.x[i]),py=toY(ds.points.y[i]);ctx.fillStyle='rgba(234,179,8,.2)';ctx.beginPath();ctx.arc(px,py,8,0,Math.PI*2);ctx.fill();ctx.fillStyle=C.oro;ctx.beginPath();ctx.arc(px,py,4,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(234,179,8,.5)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(px,py,4,0,Math.PI*2);ctx.stroke();}}
  }
  // Tangentes - vectores verdes en puntos de control
  if(cfg.tangentData&&state.layers.tangents){const td=cfg.tangentData;ctx.strokeStyle='rgba(52,211,153,.6)';ctx.lineWidth=1.5;
    for(let i=0;i<td.x.length;i++){const px=toX(td.x[i]),py=toY(td.y[i]),slope=td.dy[i],len=20,dx=len,dy=-slope*len*(ph/(yMax-yMin))/(pw/(xMax-xMin));const mag=Math.sqrt(dx*dx+dy*dy),nx=dx/mag*25,ny=dy/mag*25;
      ctx.beginPath();ctx.moveTo(px-nx,py-ny);ctx.lineTo(px+nx,py+ny);ctx.stroke();ctx.fillStyle=C.cyan;ctx.beginPath();ctx.arc(px+nx,py+ny,3,0,Math.PI*2);ctx.fill();}}
  // Curvatura - círculos mostrando radio de curvatura
  ...
  // Línea de cursor (para señal en vivo)
  if(cfg.cursorX!==undefined){const cx=toX(cfg.cursorX);ctx.strokeStyle='rgba(234,179,8,.7)';ctx.lineWidth=1.5;ctx.setLineDash([4,4]);ctx.beginPath();ctx.moveTo(cx,pad.top);ctx.lineTo(cx,pad.top+ph);ctx.stroke();ctx.setLineDash([]);
    if(cfg.cursorY!==undefined){ctx.fillStyle=C.oro;ctx.shadowColor=C.oro;ctx.shadowBlur=8;ctx.beginPath();ctx.arc(cx,toY(cfg.cursorY),5,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;}}
  // Etiquetas de ejes
  ...
  for(let i=0;i<=8;i++){const v=xMin+(xMax-xMin)*i/8;ctx.textAlign='center';ctx.fillText(v.toFixed(cfg.xDec||2),pad.left+pw*i/8,pad.top+ph+18)}
...
function drawAllCharts(){
  ...
  // Datos de tangentes en puntos de control
  const tangentData = { x: cxms, y: d.controlPoints.y, dy: cxms.map(xm=>{
    const idx = d.spline.t.reduce((best,t,i)=>Math.abs(t*1000-xm)<Math.abs(d.spline.t[best]*1000-xm)?i:best,0);
    return d.spline.dy[idx];
  })};
  // Datos de curvatura
  ...
  drawChart('canvas-spline',{
    xMin:0, xMax:per, yMin:-state.amp*1.2, yMax:state.amp*1.2,
    xLabel:'Tiempo (ms)', yLabel:'Ángulo (°)', title:'Spline Cúbico — Ciclo de Aleteo del Cóndor Andino',
    tangentData, curvData, cursorX:cursorMs, cursorY,
  ...
  drawChart('canvas-derivatives',{
    xMin:0, xMax:per, yMin:-state.amp*1.2, yMax:state.amp*1.2,
    xLabel:'Tiempo (ms)', yLabel:'Amplitud Normalizada', title:"S'(t) · S''(t) — Velocidad y Aceleración Angular", cursorX:cursorMs,
    datasets:[
      {x:xms, y:d.spline.dy.map(v=>v/dyM*state.amp), color:C.violet, lineWidth:1.5},
      {x:xms, y:d.spline.ddy.map(v=>v/ddyM*state.amp), color:C.rose, lineWidth:1.5}
    ]
  });

  // Señal temporal
  const sigDur = .5, sigPts = 400, sigT = [], sigY = [];
  const w = 2*Math.PI*state.freq;
  for(let i=0;i<sigPts;i++){
    const t = i/sigPts*sigDur;
    sigT.push(t*1000);
    sigY.push(state.amp*(Math.sin(w*t)+.08*Math.sin(2*w*t))/1.08);
  }
  drawChart('canvas-signal',{
    xMin:0, xMax:sigDur*1000, yMin:-state.amp*1.2, yMax:state.amp*1.2,
    xLabel:'Tiempo (ms)', yLabel:'Ángulo (°)', title:'Señal Temporal — Punta del Ala (Cóndor Andino)',
    cursorX:sim.playing?((sim.time%sigDur)*1000):undefined,
    cursorY:sim.playing?sim.getWingAngle(sim.time):undefined,
    datasets:[{x:sigT, y:sigY, color:C.violet, lineWidth:2}]
  });

  // Espectro de Fourier
  const fr = d.fourier.frequencies, mg = d.fourier.magnitudes, maxF = 10;
  ...
  drawChart('canvas-spectrum',{
    xMin:0, xMax:maxF, yMin:0, yMax:mMax,
    xLabel:'Frecuencia (Hz)', yLabel:'|F(ω)|', title:'Espectro — DFT (Cóndor Andino)', xDec:2,
    datasets:[{x:fS, y:mS, type:'bar', color:C.oro, hlIdx:fS.findIndex(f=>Math.abs(f-d.fourier.fundamentalFreq)<.05)}]
  });
}

// UPDATE LOOP - updates charts in real-time during simulation
let lastLiveT = 0;
function liveLoop(ts){
  if(sim.playing&&computedData){
    if(ts-lastLiveT>50){lastLiveT=ts;drawAllCharts();}
  }
  requestAnimationFrame(liveLoop);
}
requestAnimationFrame(liveLoop);

// Spline drag interactivo
let dragIdx = -1; const sC = document.getElementById('canvas-spline');
sC.addEventListener('mousedown',e=>{
  if(!computedData||!state.layers.points)return;
  const r=sC.getBoundingClientRect(),ci=sC._ci;if(!ci)return;
  const mx=e.clientX-r.left,my=e.clientY-r.top;
  for(let i=1;i<computedData.controlPoints.t.length-1;i++){
    if(Math.hypot(mx-ci.toX(computedData.controlPoints.t[i]*1000),my-ci.toY(computedData.controlPoints.y[i]))<12){
      dragIdx=i;sC.style.cursor='grabbing';return;
    }
  }
});
window.addEventListener('mousemove',e=>{
  if(dragIdx<0||!computedData)return;
  const r=sC.getBoundingClientRect(),ci=sC._ci;if(!ci)return;
  ...
  computedData.spline={t:sd.x,y:sd.y,dy:sd.dy,ddy:sd.ddy};
  computedData.coefficients=sp.coefficients.map((s,i)=>({segment:i,x0:s.x0,x1:s.x1,a:s.a,b:s.b,c:s.c,d:s.d}));
...
['btn-show-points','btn-show-tangents','btn-show-curvature'].forEach(id=>{
  document.getElementById(id).addEventListener('click',e=>{
    const l=e.currentTarget.dataset.layer;state.layers[l]=!state.layers[l];
    e.currentTarget.classList.toggle('active',state.layers[l]);drawAllCharts();
  });
});
...
// Hero CTA inicia simulación
document.getElementById('btn-start').addEventListener('click',e=>{
  e.preventDefault();
  document.getElementById('section-simulation').scrollIntoView({behavior:'smooth'});
  setTimeout(()=>{
    if(!sim.playing){
      sim.start();
      document.getElementById('icon-play').style.display='none';
...
(async function(){
  ...
  document.querySelectorAll('.section').forEach(s=>{
    s.style.opacity='0';s.style.transform='translateY(30px)';s.style.transition='opacity .8s ease,transform .8s ease';
  ...
  for(let i=0;i<25;i++){
    ...
    d.style.cssText=`position:absolute;width:${1+Math.random()*2}px;height:${1+Math.random()*2}px;background:rgba(234,179,8,${.1+Math.random()*.2});border-radius:50%;left:${Math.random()*100}%;top:${Math.random()*100}%;animation:pf ${5+Math.random()*10}s ease-in-out infinite alternate`;
    pc.appendChild(d);
  }
